...
class get_model(nn.Module):
    ...
    def forward(self, lr, info=None):
        ...
        ''' 预设输出 '''
        x_padding = torch.zeros((B, 1, u + 2, v + 2, h, w)).to(lr.device)  # [B, 1, angRes+2, angRes+2, h, w]
        x_padding[:, :, 1:u + 1, 1:v + 1, :, :] = x[:, :, :, :, :, :]
        out = torch.zeros(B, 1, u, v, h * self.factor, w * self.factor).to(lr.device)
                ...
                for threshold in range(angRes//2):
                    if distance >= (angRes//2):
                        ...
                        if (i,j)==(0,0) or (i,j)==(0, angRes-1) or (i,j)==(angRes-1, 0) or (i,j)==(angRes-1, angRes-1):
                            ''' corner '''
                            out[:, :, i, j, :, :] = self.net_corner(tmp_x)
                            break
                        else:
                            ''' side '''
                            out[:, :, i, j, :, :] = self.net_side(tmp_x)
                            break

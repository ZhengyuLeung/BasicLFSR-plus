...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        self.ReconBlock = ReconBlock(self.angRes, channels, self.scale)

    def forward(self, lr, hr=None, info=None):
        [b, c, u, v, h, w] = lr.size()
        ...
        sr_Y = self.ReconBlock(buffer_out)
        sr_Y = rearrange(sr_Y, 'b c (u h) (v w) -> b c u v h w', u=u, v=v)
        sr_YCbCr[:, 0:1, :, :, :, :] = sr_Y

        # ycbcr2rgb
        sr_rgb = LF_ycbcr2rgb(sr_YCbCr)
        out = {}
        out['SR'] = sr_rgb

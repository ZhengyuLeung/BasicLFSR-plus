...
class get_model(nn.Module):
    ...
    def forward(self, lr, info=None):
        ...
        view_n = (x.shape[1])
        image_h = int(x.shape[3])
        image_w = int(x.shape[4])  # train_data.shape[4]
        ...
        for item in position_135:
            s135_cut = x[:, item[0], item[1], :, :].view(batch_size, 1, len(item[0]), image_h, image_w)
        ...
        # residual part
        train_data = rearrange(x, 'b a1 a2 h w -> b 1 (a1 a2) h w')
        ...
        full_up += train_data
        y = rearrange(full_up, 'b c (u v) h w -> b c u v h w', u=self.angRes, v=self.angRes)
        sr_ycbcr[:, 0:1, :, :, :, :] = y

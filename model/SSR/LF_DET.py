...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        iter_path = 2
        total_depth = self.depth * iter_path
        ...
        self.upsampling = nn.Sequential(
            nn.Conv2d(self.channels * self.depth // 2, self.channels * self.factor ** 2, kernel_size=1, padding=0,
    ...
    def forward(self, lr, data_info=None):
        ...
        sr_ycbcr = LF_interpolate(lr_ycbcr, scale_factor=self.factor, mode='bicubic')

        lr_y = rearrange(lr_ycbcr[:, 0:1, ...], 'b c u v h w -> b c (u h) (v w)')
        batch, _, _, _ = lr_y.size()
        lr_upscale = interpolate(lr_y, self.angRes, scale_factor=self.factor, mode='bicubic')
        lr_y = rearrange(lr_y, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        lr_y = rearrange(lr_y, 'b c a h w -> (b a) c h w')
        # step1: Local Feature Extraction
        init_feature = self.conv_init0(lr_y)
        init_feature = self.conv_init_spa(init_feature) + init_feature

        tokens = rearrange(init_feature.flatten(2), 'b c hw -> b hw c')
        feature = rearrange(tokens, 'b (h w) c -> b c h w', h=h, w=w)
        hierarchical_feature = []
        # step2: Spatial-Angular Global Feature Extraction
        for (i, blk) in enumerate(self.blocks):
            feature = blk(feature, self.angRes)
        ...
        sr_y = final_buffer + lr_upscale
        sr_y = rearrange(sr_y, 'b c (u h) (v w) -> b c u v h w', u=self.angRes, v=self.angRes)
        sr_ycbcr[:, 0:1, :, :, :, :] = sr_y
...
class MixTransfomerBlock(nn.Module):
    ...
    def __init__(self, embed_dim, ang_num_heads, spa_num_heads, ang_mlp_ratio, spa_mlp_ratio, spa_trans_num,
                 ang_sr_ratio=1, spa_sr_ratio=1,
        ...
        for i in range(spa_trans_num):
            self.spa_Transformer_Blocks.append(
                Block(dim=embed_dim, num_heads=spa_num_heads, mlp_ratio=spa_mlp_ratio, qkv_bias=qkv_bias, drop=drop,
                      attn_drop=attn_drop, drop_path=drop_path[i], sr_ratio=spa_sr_ratio, act_layer=act_layer,
                      norm_layer=norm_layer))
        # Three parallal branches angular transform
        self.ang_Transformer_Blocks = nn.ModuleList()
        for i in range(3):
            self.ang_Transformer_Blocks.append(
                Block(dim=embed_dim, num_heads=ang_num_heads, mlp_ratio=ang_mlp_ratio, qkv_bias=qkv_bias, drop=drop,
                      attn_drop=attn_drop, drop_path=drop_path[-1], sr_ratio=ang_sr_ratio, act_layer=act_layer,
                      norm_layer=norm_layer))
        # Spatial Attention Fusion
        self.cal = nn.Conv2d(embed_dim * 3, 3, kernel_size=1, stride=1)

    def forward(self, feature, angRes):
        ...
        feature = rearrange(spa_tokens, 'b (h w) c -> b c h w', h=h, w=w)
        ...
        for num_windows in range(3):  # num_windows is M
            ...
            if (AH - ang_window_size) % ang_stride != 0:
                ...
                windows1 = torch.nn.functional.unfold(feature[:, :, :AH - rest, :AW - rest],
                                                      (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                windows2 = torch.nn.functional.unfold(feature[:, :, AH - ang_window_size:AH, :AW - rest],
                                                      (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                windows3 = torch.nn.functional.unfold(feature[:, :, :AH - rest, AW - ang_window_size:AW],
                                                      (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                windows4 = feature[:, :, AH - ang_window_size:AH, AW - ang_window_size:AW].unsqueeze(1)
                windows1 = rearrange(windows1, 'b (c k1 k2) l -> b l c k1 k2', c=C, k1=ang_window_size,
                                     k2=ang_window_size)
                num_windows1 = windows1.size()[1]
                windows2 = rearrange(windows2, 'b (c k1 k2) l -> b l c k1 k2', c=C, k1=ang_window_size,
                                     k2=ang_window_size)
                num_windows2 = windows2.size()[1]
                windows3 = rearrange(windows3, 'b (c k1 k2) l -> b l c k1 k2', c=C, k1=ang_window_size,
                                     k2=ang_window_size)
                num_windows3 = windows3.size()[1]
                num_windows4 = 1
                total_windows = torch.cat((windows1, windows2, windows3, windows4), 1)

                ang_features = self.ang_Transformer_Blocks[num_windows](
                    rearrange(total_windows, 'b l c k1 k2 -> (b l) (k1 k2) c'))
                ang_features = rearrange(ang_features, '(b l) (k1 k2) c -> b l c k1 k2 ', b=B, k1=ang_window_size,
                                         k2=ang_window_size)

                windows1 = rearrange(ang_features[:, 0:num_windows1, :, :, :], 'b l c k1 k2 -> b (c k1 k2) l')
                windows2 = rearrange(ang_features[:, num_windows1:num_windows1 + num_windows2, :, :, :],
                                     'b l c k1 k2 -> b (c k1 k2) l')
                windows3 = rearrange(
                    ang_features[:, num_windows1 + num_windows2:num_windows1 + num_windows2 + num_windows3, :, :, :],
                    'b l c k1 k2 -> b (c k1 k2) l')
                windows4 = ang_features[:, num_windows1 + num_windows2 + num_windows3:, :, :, :].squeeze(1)
                final_feature[:, :, :AH - rest, :AW - rest] += torch.nn.functional.fold(windows1,
                                                                                        (AH - rest, AW - rest), (
                                                                                        ang_window_size,
                                                                                        ang_window_size), padding=0,
                                                                                        stride=ang_stride)
                final_feature[:, :, AH - ang_window_size:AH, :AW - rest] += torch.nn.functional.fold(windows2, (
                ang_window_size, AW - rest), (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                final_feature[:, :, :AH - rest, AW - ang_window_size:AW] += torch.nn.functional.fold(windows3, (
                AH - rest, ang_window_size), (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                ...
                time1 = torch.nn.functional.unfold(time1, (ang_window_size, ang_window_size), padding=0,
                                                   stride=ang_stride)
                total_times[:, :, :AH - rest, :AW - rest] += torch.nn.functional.fold(time1, (AH - rest, AW - rest), (
                ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                time2 = torch.ones_like(torch.empty(1, C, ang_window_size, AW - rest)).cuda()
                time2 = torch.nn.functional.unfold(time2, (ang_window_size, ang_window_size), padding=0,
                                                   stride=ang_stride)
                total_times[:, :, AH - ang_window_size:AH, :AW - rest] += torch.nn.functional.fold(time2, (
                ang_window_size, AW - rest), (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                time3 = torch.ones_like(torch.empty(1, C, AH - rest, ang_window_size)).cuda()
                time3 = torch.nn.functional.unfold(time3, (ang_window_size, ang_window_size), padding=0,
                                                   stride=ang_stride)
                total_times[:, :, :AH - rest, AW - ang_window_size:AW] += torch.nn.functional.fold(time3, (
                AH - rest, ang_window_size), (ang_window_size, ang_window_size), padding=0, stride=ang_stride)
                total_times[:, :, AH - ang_window_size:AH, AW - ang_window_size:AW] += 1
                features.append(final_feature / total_times)
            else:
                windows = torch.nn.functional.unfold(feature, (ang_window_size, ang_window_size), padding=0,
                                                     stride=ang_stride)
                windows = rearrange(windows, 'b (c k1 k2) l -> (b l) c k1 k2', c=C, k1=ang_window_size,
                                    k2=ang_window_size)
                ang_features = self.ang_Transformer_Blocks[num_windows](rearrange(windows, 'b c k1 k2 -> b (k1 k2) c'))
                ang_features = rearrange(ang_features, '(b l) (k1 k2) c -> b (c k1 k2) l', b=B, k1=ang_window_size,
                                         k2=ang_window_size)
                final_feature = torch.nn.functional.fold(ang_features, (AH, AW), (ang_window_size, ang_window_size),
                                                         padding=0, stride=ang_stride)
                time = torch.ones_like(torch.empty(1, C, AH, AW)).cuda()
                time = torch.nn.functional.unfold(time, (ang_window_size, ang_window_size), padding=0,
                                                  stride=ang_stride)
                times = torch.nn.functional.fold(time, (AH, AW), (ang_window_size, ang_window_size), padding=0,
                                                 stride=ang_stride)
        ...
        final_feature = rearrange(final_feature, 'b c (h a1) (w a2) -> b c (a1 a2) h w', a1=angRes, a2=angRes, h=h, w=w)

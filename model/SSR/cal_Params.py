...
if __name__ == "__main__":
    ...
    input = torch.randn([1, 3, args.angRes_in, args.angRes_in, patch_size, patch_size]).to(device)
    GPU_cost = torch.cuda.memory_allocated(0)
    out = net(input)
    GPU_cost1 = (torch.cuda.memory_allocated(0) - GPU_cost) / 1024 / 1024 / 1024  # GB
    GPU_cost_max = (torch.cuda.max_memory_allocated(0)) / 1024 / 1024 / 1024  # GB
    print('   GPU max consumption: %.4fGB' % (GPU_cost_max))
    ...
    print('   Time used: %.4fms' % ((end - start) / 100 * 1000))
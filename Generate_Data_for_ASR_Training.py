...
def main(args):
    ...
    patch_size = 70
    # stride = 16 # RE_HCI 的 stride 设为16，因为 RE_HCI 的数量少
    #             # RE_Lytro 的 stride 设为32，这样两个数据集的数量都在15000左右
    ...
    for index_dataset in range(len(src_datasets)):
        if src_datasets[index_dataset] not in ['RE_HCI', 'RE_Lytro']:
            continue
        ...
        if name_dataset == 'RE_HCI':
            stride = 16
        elif name_dataset == 'RE_Lytro':
            stride = 32
            ...
            for file in files:
                ...
                for h in range(0, H - patch_size + 1, stride):
                    for w in range(0, W - patch_size + 1, stride):
                        ...
                        HR_LF = np.zeros((U, V, patch_size, patch_size, 3), dtype='single')

                        HR_LF[:, :, :, :, :] = LF[:, :, h: h + patch_size, w: w + patch_size, :]
